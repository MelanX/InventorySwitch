...
public class Events {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (playerPositions.get(player) == null) playerPositions.put(player, new Vec3d(player.posX, player.posY, player.posZ));
        if (hasEffect(player, Registration.freeze.get())) {
            Vec3d pos = playerPositions.get(player);
            player.setPosition(pos.getX(), pos.getY(), pos.getZ());
        } else {
            playerPositions.put(player, new Vec3d(player.posX, player.posY, player.posZ));
        }
    }
